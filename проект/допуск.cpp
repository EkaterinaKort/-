...
public:
    ...
    // Заполнение массива для тестов чтоб не вводить каждый раз. Заполняет рандомными числами
    void FillingArray(Book Books[])
    {
        for (int i = 0; i < Book::SIZE; i++)
        {
            Books[i].id = to_string(i + 1);
            Books[i].title = to_string(rand() % 10);
            Books[i].autor = to_string(rand() % 10);
            Books[i].publishing = to_string(rand() % 10);
            Books[i].genre = to_string(rand() % 10);
        }
    }
    // вывод в консоль 
    void Print(Book Books)
    {
        if (Books.id == to_string(1))
        {
            cout.width(7);
